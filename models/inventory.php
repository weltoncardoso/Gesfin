...
public function searchProductByName($name, $id_company) {
   ...
   $sql = $this->db->prepare("SELECT name, price, id FROM inventory WHERE name LIKE :name AND id_company = :id_company LIMIT 15");
   $sql->bindValue(':name', '%'.$name.'%');
   $sql->bindValue(':id_company', $id_company);
   $sql->execute();
  ...
  public function increaseInventory($id_prod, $id_company, $quant_prod, $id_user) {
     $sql = $this->db->prepare("UPDATE inventory SET quant = quant + $quant_prod WHERE id = :id AND id_company = :id_company");
         ...
         $sql->bindValue(':id', $id_prod);
          ...
          $this->setLog($id_prod, $id_company, $id_user, 'iwn');
  }

    public function decreaseInventory($id_prod, $id_company, $quant_prod, $id_user) {
     $sql = $this->db->prepare("UPDATE inventory SET quant = quant - $quant_prod WHERE id = :id AND id_company = :id_company");
